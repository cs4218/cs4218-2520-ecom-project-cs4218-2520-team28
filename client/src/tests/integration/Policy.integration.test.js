...
// Frontend Integration Tests — Policy Page
//
// ────────────────────────────────────────────────────────────────────────────
//  TEST LEVEL OVERVIEW
// ────────────────────────────────────────────────────────────────────────────
...
//
//  Level 3 — Policy + real Layout + real Header + real Footer     [9 tests]
//    → IMPLEMENTED BELOW
//    Scope: <Policy /> rendered with REAL Layout, Header, Footer, and all
//           their context dependencies (AuthProvider, CartProvider,
//           SearchProvider). Network (axios) is mocked at the boundary.
//    What is tested:
//      • Page content: heading, paragraph text, image, document.title
...
//
//  Level 4 — Full App routing to /policy                           [9 tests]
//    → IMPLEMENTED BELOW
//    Scope: <App /> in MemoryRouter at /policy, with REAL providers.
//    What is tested:
//      • Route /policy renders Policy page content
//      • Header shows correct auth state (logged-out: Login + Register)
//      • Footer "Contact" link navigates to /contact (cross-page routing)
//      • After navigating away, Policy content is no longer visible
//      • Navbar brand link renders (App > Policy > Layout > Header > brand)
...
//      • document.title is "Privacy Policy" via full Helmet chain
//
// ────────────────────────────────────────────────────────────────────────────
//  TOTAL: 18 test cases
// ────────────────────────────────────────────────────────────────────────────
...
/**
 * Seeds localStorage with auth and cart state, then renders <Policy /> wrapped
 ...
 * starting at /policy, with the real provider chain.
 */
function renderAppAtPolicy({
  user = null,
  token = "",
  cartItems = [],
  categories = [],
...
// ─────────────────────────────────────────────────────────────────────────────
//  LEVEL 3 — Policy + real Layout + real Header + real Footer
// ─────────────────────────────────────────────────────────────────────────────
describe("Level 3 — Policy + real Layout + real Header + real Footer", () => {
  // ── Page content ────────────────────────────────────────────────────────

  it("L3.1 renders the PRIVACY POLICY heading", () => {
    renderPolicy();
    expect(screen.getByRole("heading", { name: /privacy policy/i })).toBeInTheDocument();
  });

  it("L3.2 renders privacy policy paragraph text", () => {
    renderPolicy();
    expect(screen.getByText(/committed to protecting/i)).toBeInTheDocument();
  });

  it("L3.3 renders policy image with correct src and alt", () => {
    renderPolicy();
  ...
  it("L3.4 sets document title to 'Privacy Policy'", async () => {
    renderPolicy();
    await waitFor(() => {
  ...
  it("L3.6 Header shows user name and /dashboard/user link, badge shows item count", async () => {
    renderPolicy({
      user: { _id: "u2", name: "Test User", role: 0 },
      token: "tok",
      cartItems: [{ _id: "p1" }, { _id: "p2" }, { _id: "p3" }],
    });
    await waitFor(() => {
  ...
  it("L3.7 Header Dashboard link points to /dashboard/admin for admin user (role 1)", async () => {
    renderPolicy({
      user: { _id: "a2", name: "Admin User", role: 1 },
      token: "admintok",
    });
    await waitFor(() => {
  ...
  it("L3.8 Header shows category from useCategory in dropdown after axios resolves", async () => {
    renderPolicy({
      categories: [{ name: "Books", slug: "books" }],
    });
    await waitFor(() => {
...
// ─────────────────────────────────────────────────────────────────────────────
//  LEVEL 4 — Full App routing to /policy
// ─────────────────────────────────────────────────────────────────────────────
describe("Level 4 — Full App routing to /policy", () => {
  it("L4.1 route /policy renders Privacy Policy heading and content", async () => {
    renderAppAtPolicy();
    await waitFor(() => {
      expect(
  ...
  it("L4.2 Header shows Login and Register links when logged out on /policy", async () => {
    renderAppAtPolicy();
    await waitFor(() => {
      expect(screen.getByRole("link", { name: /login/i })).toBeInTheDocument();
    });
  ...
  it("L4.3 clicking Footer Contact link navigates to /contact page", async () => {
    renderAppAtPolicy();
    // Wait for Policy page to be visible
    await waitFor(() => {
      expect(
        screen.getByRole("heading", { name: /privacy policy/i })
      ).toBeInTheDocument();
    ...
    // Contact page heading should now appear
    await waitFor(() => {
      expect(
  ...
  it("L4.4 after navigating to /contact via Footer link, Policy heading is gone", async () => {
    renderAppAtPolicy();
    await waitFor(() => {
      expect(
        screen.getByRole("heading", { name: /privacy policy/i })
  ...
  // App → Route → Policy → Layout → Header/Footer and their own children
  // are all correctly mounted when the route is resolved by App.

  it("L4.5 Header navbar brand link renders (App > Policy > Layout > Header > brand)", async () => {
    renderAppAtPolicy();
    await waitFor(() => {
      expect(
        screen.getByRole("heading", { name: /privacy policy/i })
  ...
  it("L4.6 SearchInput form renders inside Header (App > Policy > Layout > Header > SearchInput)", async () => {
    renderAppAtPolicy();
    await waitFor(() => {
      expect(
        screen.getByRole("heading", { name: /privacy policy/i })
  ...
  it("L4.7 Header Home nav link renders (App > Policy > Layout > Header > nav)", async () => {
    renderAppAtPolicy();
    await waitFor(() => {
      expect(
        screen.getByRole("heading", { name: /privacy policy/i })
  ...
  it("L4.8 Footer copyright text renders (App > Policy > Layout > Footer > h4)", async () => {
    renderAppAtPolicy();
    await waitFor(() => {
      expect(
        screen.getByRole("heading", { name: /privacy policy/i })
  ...
  it("L4.9 document.title is 'Privacy Policy' through full App > Policy > Layout > Helmet chain", async () => {
    renderAppAtPolicy();
    await waitFor(() => {
      expect(document.title).toBe("Privacy Policy");
    });
